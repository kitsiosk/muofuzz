...
void print_array_main(u32 *array, u32 size) {
    printf("[");
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  u32 mut_max = 37;
  afl->finds_per_mutator = (u32 *)malloc(mut_max * sizeof(u32)); // 37 mutators
  for (u32 i=0; i<mut_max; ++i){ // all mutators start with 0 finds
    afl->finds_per_mutator[i] = 0;
  }
...
stop_fuzzing:
  print_stage_stats(afl);
  print_array_main(afl->finds_per_mutator, 37);

...
u8 fuzz_one_original(afl_state_t *afl) {
  double random_double = (double)rand() / RAND_MAX; // to print or not for this seed
  double random_threshold = 0.00001;
  u32 len, temp_len;
  u32 j;
  u32 i;
...
havoc_stage:
  ...
  u32 *selected_mutators = (u32 *)malloc(stack_max * sizeof(u32)); // Allocate memory for selected_mutators array
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (mydebug) printf("use_stacking=%d\n", use_stacking);
    for (i = 0; i < use_stacking; ++i) {
      // KK: deleted some code about custom mutators from here
    ...
    retry_havoc_step: {
      ...
      //u32 r = sample_from_distribution(afl, finds_per_mutator_normalized, rand_max);

      selected_mutators[i] = mutation_array[r];
    ...
    if (afl->queued_items != havoc_queued) {
      ...
      for (i=0; i<use_stacking; ++i){
        afl->finds_per_mutator[selected_mutators[i]] += 1;
      }
  ...
  if (random_double < random_threshold){
      print_u32_array(afl->finds_per_mutator, MUT_MAX);
  }

  // free(finds_per_mutator);

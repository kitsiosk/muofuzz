...
int sample_from_distribution(afl_state_t *afl, int prev_prev_mutator, int prev_mutator) {
    // prev_prev_mutator is the index of mutator m_{n-2} 
    // prev_mutator is the index of mutator m_{n-1}
    // we want to sample mutator m_{n}
    int i = rand() % mut_max;
    if (rand() / (double) RAND_MAX < afl->prob_table_mut[prev_prev_mutator][prev_mutator][i])
        return i;
    else
        return afl->alias_table_mut[prev_prev_mutator][prev_mutator][i];
}
...
havoc_stage:
  ...
  int prev_mutator, prev_prev_mutator;
  u32 num_of_available_stacks = 1<<afl->havoc_stack_pow2;

  // for training use only trigrams. After training use default Nstacked
  if (afl->in_training){
    use_stacking      = 3;
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    retry_havoc_step: {
      ...
      if (afl->in_training) {
        r = rand_below(afl, mut_max);
          ...
          if (prev_prev_mutator == -1){      // if we did not yet select the first two mutators in the sequence
            ...
            r = sample_from_distribution(afl, prev_prev_mutator, prev_mutator);
          }
      }
      ...
      // more to the right
      prev_prev_mutator = prev_mutator;
      prev_mutator = r;
    ...
    if (afl->queued_items != havoc_queued) {
      ...
      if (afl->in_training){
        // increase the number of finds of this trigram
        afl->finds_per_mutator[selected_mutators[0]][selected_mutators[1]][selected_mutators[2]] += 1;
      }else{

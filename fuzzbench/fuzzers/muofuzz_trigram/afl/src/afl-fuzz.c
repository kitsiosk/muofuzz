...
void update_distribution(afl_state_t *afl, double ***probabilities, u32 num_rows, u32 num_cols) {
    ...
    for (u32 row = 0; row < num_rows; row++) { // dimension of the first mutator
      for (u32 row2 = 0; row2 < num_rows; row2++){ // dimension of the second mutator
        ...
        for (u32 i = 0; i < num_cols; ++i) {
            scaled_prob[i] = probabilities[row][row2][i] * num_cols; // probabilities[row][row2] is now a probability distribution
            //printf("2\n");
        ...
        while (small_size > 0 && large_size > 0) {
            u32 l, g;
            if (small_size >= 0 && small_size < num_cols){
              l = small[--small_size];
            }else{
              printf("Prob1. Small size = %d\n", small_size);
              l = small[--small_size];
            }

            if (large_size >= 0 && small_size<num_cols){
              g = large[--large_size];
            }else{
              printf("Prob2. Large size = %d\n", large_size);
              g = large[--large_size];
            }
        ...
        while (small_size > 0){
            //printf("small_size=%d\n", small_size);
            //small_size--;
            //printf("small[--small_size]=%d\n", small[small_size]);
            //printf("prob[small[--small_size]]=%f\n", prob[small[small_size]]);
        ...
        //printf("3\n");
        afl->alias_table_mut[row][row2] = alias;
        //printf("4\n");
        afl->prob_table_mut[row][row2]  = prob;
        //printf("5\n");
...
void print_u32_array_(u32 ***array, u32 size) {
    printf("[");
    for (u32 ifirst = 0; ifirst < size; ++ifirst) {
      printf("[");
      for (u32 i = 0; i < size; ++i) {
          printf("[");
          for (u32 j = 0; j < size; ++j) {
              printf("%d", array[ifirst][i][j]);
              if (j < size - 1) {
                  printf(", ");
              }
          }
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  for (u32 i = 0; i < mut_max; ++i) {
      ...
      afl->alias_table_mut[i]   = (u32 **)malloc(mut_max * sizeof(u32 *));
      afl->prob_table_mut[i]    = (double **)malloc(mut_max * sizeof(double *));

      for (u32 j = 0; j < mut_max; ++j) {
          afl->finds_per_mutator[i][j] = (u32 *)malloc(mut_max * sizeof(u32));
          afl->mut_probabilities[i][j] = (double *)malloc(mut_max * sizeof(double));

        for (u32 k = 0; k < mut_max; ++k) {
            afl->finds_per_mutator[i][j][k] = 0;
        }
      }
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      if (afl->in_training && get_cur_time() - afl->start_time > training_hours * 60 * 60 * 1000){
        ...
        // for each mutator trigram, set Pij = finds/sum(finds) (normalized)
        for (u32 iii = 0; iii < num_rows; ++iii){ 
        for (u32 ii = 0; ii < num_rows; ++ii){
            ...
            for (u32 j = 0; j < num_cols; j++) {
                afl->mut_probabilities[iii][ii][j] = (double)(afl->finds_per_mutator[iii][ii][j]);
                sum += afl->mut_probabilities[iii][ii][j];
            }

            // It is possible that all finds per mutator are 0, in which case we randomly initialize the distribution
            if (sum < epsilon){
              // Generate random values for each element in the probability distribution
              sum = 0.0;
              for (u32 j = 0; j < num_cols; j++) {
                  afl->mut_probabilities[iii][ii][j] = (double)rand() / RAND_MAX;
                  sum += afl->mut_probabilities[iii][ii][j];
              }
            }
            ...
            for (u32 j = 0; j < num_cols; j++) {
                afl->mut_probabilities[iii][ii][j] /= (sum + epsilon);

    ...
    do {
      ...
      if (afl->in_training && get_cur_time() - afl->start_time > training_hours * 60 * 60 * 1000){
        ...
        for (u32 ii = 0; ii < num_rows; ++ii){
            ...
            // For this ablation study, we ignore whatever exists in afl->finds_per_mutator
            // and initialize a random probability array to test the importance (effect) of 
            // "afl->finds_per_mutator"
            if (sum < epsilon){

...
int main(int argc, char **argv_orig, char **envp) {
  ...
  afl->total_finds_per_mutator = (double *)malloc(mut_max * sizeof(double *)); // for greedy start
  ...
  for (u32 i = 0; i < mut_max; ++i) {
      ...
      afl->total_finds_per_mutator[i] = 0.0;
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      if (afl->in_training && get_cur_time() - afl->start_time > training_hours * 60 * 60 * 1000){
        ...
        double epsilon = 1e-10; // Regularization constant
        u32 sum_of_total_finds_per_mutator = 0;
        ...
        for (u32 ii = 0; ii < num_rows; ++ii){
            ...
            // Also, compute the number of total finds per mutator, which is 
            // sum(columns) + sum(rows) - Fii
            for (u32 j = 0; j < num_cols; j++) {
              afl->total_finds_per_mutator[ii] += afl->finds_per_mutator[ii][j];
              afl->total_finds_per_mutator[ii] += afl->finds_per_mutator[j][ii];
            }
            afl->total_finds_per_mutator[ii] -= afl->finds_per_mutator[ii][ii]; // substract since we counted this twice.
            sum_of_total_finds_per_mutator += afl->total_finds_per_mutator[ii];

        }

        // normalize total_finds_per_mutator so we can sample from it
        for (u32 ii = 0; ii < num_rows; ++ii){
          afl->total_finds_per_mutator[ii] = afl->total_finds_per_mutator[ii]/(sum_of_total_finds_per_mutator + epsilon);
        }

        printf("P: \n");
        print_double_array_(afl->mut_probabilities, num_rows);

        printf("Total finds per mutator normalized: \n");
        print_double_array_1d(afl->total_finds_per_mutator, num_rows);
...
stop_fuzzing:
  ...
  print_double_array_1d(afl->total_finds_per_mutator, mut_max);

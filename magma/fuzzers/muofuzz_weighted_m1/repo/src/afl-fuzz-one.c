...
// For Greedy start
int sample_from_distribution_1d(double *array, int size) {
    ...
    retry_havoc_step: {
      ...
      if (afl->in_training) {
        r = rand_below(afl, mut_max);
          ...
          if (prev_mutator == -1) {
            // sample from total_finds_per_mutator
            r = sample_from_distribution_1d(afl->total_finds_per_mutator, mut_max);
          } else {

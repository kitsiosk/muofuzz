...
havoc_stage:
  ...
  if (afl->in_training){
    ...
    // define stack_max here, and use_stacking will be randomly selected in each
    // loop iteration below
    stack_max         = 1 << (1 + rand_below(afl, afl->havoc_stack_pow2));
    selected_mutators = (u32 *)malloc(stack_max * sizeof(u32));
  }

  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    // if in training, we have already defined use_stacking=2
    if (!afl->in_training){
